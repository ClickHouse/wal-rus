...
s3) DST="s3://${WALRS_S3_BUCKET:-walrs}/$(basename "$WORKROOT")-copy" ;;
gcs) DST="gs://${WALRS_GS_BUCKET:-walrs}/$(basename "$WORKROOT")-copy" ;;
*)
...
s3) WALG_S3_PREFIX="$DST" walrs backup-list | tee "$WORKROOT/copy-list.txt" ;;
gcs) WALG_GS_PREFIX="$DST" walrs backup-list | tee "$WORKROOT/copy-list.txt" ;;
esac
