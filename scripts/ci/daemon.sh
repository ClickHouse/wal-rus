...
walrs daemon --socket "$SOCKET" >"$WORKROOT/daemon.log" 2>&1 &
DAEMON_PID=$!
...
# CLI client paths
walrs daemon-client --socket "$SOCKET" check
walrs daemon-client --socket "$SOCKET" wal-push "$PGDATA/pg_wal/$WAL"

# Verify the push actually landed by fetching via direct CLI (not daemon).
DST="$WORKROOT/fetched_wal"
walrs wal-fetch "$WAL" "$DST"
...
walrs daemon-client --socket "$SOCKET" wal-fetch "$WAL" "$DST2"
