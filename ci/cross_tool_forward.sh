#!/usr/bin/env bash
# Forward bucket interop: walrus writes the backup, wal-g reads it.
...
# wal-g must see the backup written by walrus
walg backup-list | tee "$WORKROOT/walg-list.txt"
grep -E '^base_' "$WORKROOT/walg-list.txt" || { echo "wal-g cannot see walrus backup"; exit 1; }
