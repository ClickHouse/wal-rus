...
# hba method demands a client cert whose CN maps to the PG user; walross presents
...
openssl req -x509 -newkey rsa:2048 -nodes -days 2 \
    -keyout "$CERTS/rogue-ca.key" -out "$CERTS/rogue-ca.crt" -subj '/CN=walross-rogue-ca'
...
PGSSLCERT="$CERTS/client.crt" PGSSLKEY="$CERTS/client.key" walross backup-push

echo "== no client cert: must fail closed (server demands one) =="
if walross backup-push 2>"$WORKROOT/nocert.err"; then
...
if PGSSLCERT="$CERTS/rogue.crt" PGSSLKEY="$CERTS/rogue.key" walross backup-push 2>"$WORKROOT/rogue.err"; then
