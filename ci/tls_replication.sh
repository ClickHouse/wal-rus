...
PGSSLMODE=verify-full PGSSLROOTCERT="$CERTS/ca.crt" wal-rs backup-push

echo "== verify-ca with correct root: must succeed =="
PGSSLMODE=verify-ca PGSSLROOTCERT="$CERTS/ca.crt" wal-rs backup-push

echo "== verify-full with WRONG root: must fail closed =="
if PGSSLMODE=verify-full PGSSLROOTCERT="$CERTS/wrong.crt" wal-rs backup-push 2>"$WORKROOT/wrong.err"; then
...
PGSSLMODE=require wal-rs backup-push
...
PGSSLMODE=require PGSSLROOTCERT="$CERTS/ca.crt" wal-rs backup-push

echo "== require with WRONG root: must fail closed =="
if PGSSLMODE=require PGSSLROOTCERT="$CERTS/wrong.crt" wal-rs backup-push 2>"$WORKROOT/require_wrong.err"; then
