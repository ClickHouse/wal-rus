...
# replication connection over TCP, so PGSSLMODE drives walrus's maybe_upgrade.
...
openssl req -x509 -newkey rsa:2048 -nodes -days 2 \
    -keyout "$CERTS/wrong.key" -out "$CERTS/wrong.crt" -subj '/CN=walrus-wrong-ca'
...
PGSSLMODE=verify-full PGSSLROOTCERT="$CERTS/ca.crt" walrus backup-push

echo "== verify-ca with correct root: must succeed =="
PGSSLMODE=verify-ca PGSSLROOTCERT="$CERTS/ca.crt" walrus backup-push

echo "== verify-full with WRONG root: must fail closed =="
if PGSSLMODE=verify-full PGSSLROOTCERT="$CERTS/wrong.crt" walrus backup-push 2>"$WORKROOT/wrong.err"; then
...
PGSSLMODE=require walrus backup-push
...
PGSSLMODE=require PGSSLROOTCERT="$CERTS/ca.crt" walrus backup-push

echo "== require with WRONG root: must fail closed =="
if PGSSLMODE=require PGSSLROOTCERT="$CERTS/wrong.crt" walrus backup-push 2>"$WORKROOT/require_wrong.err"; then
