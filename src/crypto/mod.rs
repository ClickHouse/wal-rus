...
//! (`WALG_PGP_KEY` / `_PATH` / `_PASSPHRASE`). walrus ships libsodium only,
//! by design:
//!
...
//! re-encrypt to libsodium before switching to walrus (or stay on wal-g).
...
pub fn forbid_pgp(vars: &crate::config::Vars) -> Result<()> {
    ...
    if !set.is_empty() {
        bail!(
            "OpenPGP encryption is not supported by walrus ({set:?} set). \
